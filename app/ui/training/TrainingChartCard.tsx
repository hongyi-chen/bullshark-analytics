...
export default function TrainingChartCard({ athletes, loading }: TrainingChartCardProps) {
  ...
  return (
    <Card fixedTall style={{ height: '600px' }}>
      ...
      />
      <div className={`flexFill ${css.chartArea}`} role="img" aria-label={`Multi-line chart showing weekly training kilometers for ${athletes.length} athlete${athletes.length !== 1 ? 's' : ''}. Click legend items to focus on specific athletes.`}>
